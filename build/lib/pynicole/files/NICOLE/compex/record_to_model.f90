...
Subroutine Record_to_model_2comp(np, Atmo, Vector1, Vector2, KeepVars, dir)
   ...
   Real, Dimension(nvarsdepth*np+nvarssingle) :: Vector1p
   Real, Dimension(nvarsdepth*np+nvarssingle) :: Vector2
   Real, Dimension(8) :: KeepVars
   !
   ! Copy the second "row" of the vector1 to the first one, to fit the input of z (Mingjie)

   Call Record_to_model(np, Atmo%Comp1, Vector1, dir)
   ! print *, 'Inside Record_to_model_2comp'
   ! print *, Atmo%Comp1%z_scale
...
Subroutine Record_to_model(np, Atmo, Vector, dir)
   ...
   !
   If (dir .eq. 1) then
      ! Copy the second "row" of the vector to the first one, to fit the input of z (Mingjie)
      ! Vector(1:np) = Vector(np+1:2*np) 
