...
!
  Do ipoint=2, Params%n_points
   ...
   !   dtau = 10**dtau
     ...
     print *,'ipoint=',ipoint, ', kappa=', kappa(ipoint), ', rho=', atmo%rho(ipoint)
   !   print *, 'kappa*rho(i-1) + kappa*rho(i)=', kappa(ipoint-1)*atmo%rho(ipoint-1) +  kappa(ipoint)*atmo%rho(ipoint)
   !   print *, 'Z(i) =', (Atmo%z_scale(ipoint))*1.e5
     print *,'   tau0, dtau, tau=',atmo%ltau_500(ipoint)-dtau,dtau,atmo%ltau_500(ipoint)

  End do

  ! Change the first tau to the one exptrapolated from the 2nd and 3rd.
  Atmo%ltau_500(1) = Atmo%ltau_500(2) - (Atmo%ltau_500(3) - Atmo%ltau_500(2))
!
