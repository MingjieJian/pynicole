...
# This file is adopted and revided from NICOLE repo.
...
# Return nx, ny and nz
    ...
    if string.decode() == 'nicole1.6bm': # Older format version
        filetype='nicole1.6'
        nx=1
        [string,ny,nz]=struct.unpack('<16s'+intf+intf,header)
        f.close()
...
#            sys.exit(1)
    ...
    if string.decode() == 'nicole2.3bm' or string.decode() == 'nicole2.3bp': # Old format version
        filetype='nicole2.3'
...
#            sys.exit(1)
        return [filetype,nx,ny,nz]
    if string.decode() == 'nicole2.6bm': # Old format version
        filetype='nicole2.6'
        [string,nx,ny,nz]=struct.unpack('<16s'+int4f+int4f+intf,header)
        f.close()
        filesize=os.path.getsize(filename)
...
#            sys.exit(1)
        return [filetype,nx,ny,nz]
    if string.decode() == 'nicole18.04' or string.decode() == 'nicole1804m': # Current format version
...
# Return nx, ny and nlambda
    ...
    fdata = codecs.open('/media/disk/software/NICOLE/run/modelout.pro', 'r', encoding='utf-8', errors='ignore')
    readstr = fdata.read()
    fdata.close()
#     readstr = f.read(100000)
    f.close()
    nprintable=0
    for char in readstr: 
    ...
    # If printable characters occupies > 95%, then it is ASCII file. 
    ...
    if readstr.decode() == 'nicole1.6bp': # Older format version
        filetype='nicole1.6'
        nx=1
        [readstr,ny,nlam]=struct.unpack('<16s'+intf+intf,header)
        f.close()
...
#            sys.exit(1)
        return [filetype,nx,ny,nlam]
    if readstr.decode() == 'nicole2.3bp': # Current format version
        filetype='nicole2.3'
...
#            sys.exit(1)
    ...
    for i in range(len(stri)): 
        bytes.append(ord(stri.decode()[i]))
    f.close()
    if bytes == [83, 82, 0, 4, 0, 0, 0, 10, 0, 0]:
        filetype='idl'
        try:
